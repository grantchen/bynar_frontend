...
exports.handler = async function(event, context, callback) {
  ...
  if (!event.username || !event.full_name || !event.postal_code || !event.country_ci || !event.organization_name || !event.tax_number || !event.country_ti) {
    ...
    cognitoidentityserviceprovider.adminListGroupsForUser(getUserParams, async function(err, data) {
      if (err) {
        var result = {
          statusCode: 400,
        ...
        if (data.Groups.length == 0) {
          ...
          var mysqlResult = await connection.awaitQuery("SELECT * FROM bynar.accounts where username='test123';");
          ...
          if (mysqlResult.length !== 0) {
            ...
            const userGroupName = data.Groups[0].GroupName;
